...
import boto3,json,datetime,string

import random,argparse,botocore
...
try:
    ...
    if error.response['Error']['Code'] == 'InvalidToken':
        print("Please updateyour aws credentials with a valid token")
    else:
        print(f"Some other error occurred {error}")
except s3client.meta.client.exceptions.BucketAlreadyExists as err:
